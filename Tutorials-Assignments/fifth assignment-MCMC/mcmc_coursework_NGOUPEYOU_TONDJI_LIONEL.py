...
def sample_beta_0(y, x, beta_1, beta_2, tau, mu_0, tau_0):
    ...
    precision = N*tau + tau_0
    mean = ((tau * sum(y - beta_1*x - beta_2*(x**2))) + (tau_0 * mu_0))/ precision
...
def sample_beta_1(y, x, beta_0, beta_2, tau, mu_1, tau_1):
    ...
    mean = (tau * sum((y - beta_0 - beta_2*(x**2)) * x ) + tau_1 * mu_1)/ precision
...
def sample_beta_2(y, x, beta_0, beta_1, tau, mu_2, tau_2):
    ...
    mean = (tau * sum((y - beta_0 - beta_1 * x) * (x**2) ) + tau_2 * mu_2)/ precision
    return np.random.normal(mean, 1 / np.sqrt(precision))
...
def sample_tau(y, x, beta_0, beta_1, beta_2, alpha, beta):
    ...
    alpha_new = N/2 + alpha
    beta_new = beta + 0.5 * sum(pow(y - beta_0 - beta_1*x - beta_2*(x**2), 2))
